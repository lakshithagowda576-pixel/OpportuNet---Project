...
export default function AdminPanel() {
  ...
  const filteredUsers = (allUsers as any[]).filter((u: any) => {
    ...
    return !/^user\d+$/.test(name) && !/^user\d+@/.test(email) && !/^testuser\d*$/.test(name);
  });
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      {activeTab === "users" && (
        <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
          <div className="p-5 border-b border-border">
            <h3 className="font-bold text-base flex items-center gap-2"><Users className="w-5 h-5 text-primary" /> Registered Users ({filteredUsers.length})</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-secondary/50">
