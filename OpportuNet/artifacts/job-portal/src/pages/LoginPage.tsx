    ...
    <div className="min-h-screen bg-gradient-to-br from-primary/5 via-accent/5 to-background flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <AnimatePresence mode="wait">
          <motion.div
            key={mode}
            initial={{ opacity: 0, rotateY: -12, y: 16, scale: 0.98 }}
            animate={{ opacity: 1, rotateY: 0, y: 0, scale: 1 }}
            exit={{ opacity: 0, rotateY: 12, y: -16, scale: 0.98 }}
            transition={{ duration: 0.32, ease: "easeOut" }}
            className="bg-card rounded-3xl shadow-2xl shadow-primary/10 border border-border overflow-hidden"
          >
            {/* Tabs */}
