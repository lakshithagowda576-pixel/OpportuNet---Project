...
import { sendAllJobAlertDigests } from "./lib/job-alert-service";
import { ingestAllActiveSources } from "./lib/job-ingestion-service";
...
export function setupCronJobs() {
  ...
  // Job Alerts Digests - Daily at 6:30 AM
  cron.schedule("30 6 * * *", async () => {
    logger.info("Running daily job alerts digest task...");
    try {
      await sendAllJobAlertDigests("daily");
    } catch (error) {
      logger.error(error, "Failed to run daily job alerts digest task");
    }
  });

  // Job Alerts Digests - Weekly on Monday at 6:30 AM
  cron.schedule("30 6 * * 1", async () => {
    logger.info("Running weekly job alerts digest task...");
    try {
      await sendAllJobAlertDigests("weekly");
    } catch (error) {
      logger.error(error, "Failed to run weekly job alerts digest task");
    }
  });
  ...
  // Job Data Ingestion - Twice daily (6 AM and 2 PM)
  cron.schedule("0 6,14 * * *", async () => {
    logger.info("Running job data ingestion task...");
    try {
      const results = await ingestAllActiveSources("software engineer");
      logger.info({ ingestionResults: results }, "Job ingestion completed");
    } catch (error) {
      logger.error(error, "Failed to run job ingestion task");
    }
  });
