...
// POST /api/applications/direct - Store application from branded page
router.post("/applications/direct", upload.single("resume"), async (req, res) => {
  try {
    ...
    try {
      await sendApplicationConfirmationEmail(app.id);
    } catch (emailError) {
    ...
    res.status(201).json({ 
      ...
      application: { ...app, appliedAt: app.appliedAt.toISOString() }
    });
  } catch (error: any) {
